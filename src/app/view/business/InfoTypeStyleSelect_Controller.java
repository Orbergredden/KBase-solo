 ...
 */
public class InfoTypeStyleSelect_Controller {
	/**
	...
	private InfoTypeStyleItem styleLast;
	private InfoTypeStyleItem styleInList;
     ...
     */
    public InfoTypeStyleSelect_Controller () {         }
    ...
    private void initControlsValue() {
    	prefs = Preferences.userNodeForPackage(InfoTypeStyleSelect_Controller.class);
    	
    	//-------- get default style 
    	styleDefault = conn.db.infoTypeStyleGetDefault(themeId, infoTypeId);
    	...
    	//-------- get last style
    	styleLast = conn.db.infoTypeStyleGetCurrent(themeId, infoTypeId, 1);
    	...
    	treeTableColumn_id.setPrefWidth(prefs.getDouble("InfoTypeStyleSelect__treeTableColumn_id__PrefWidth", 75));

    	treeTableColumn_id.widthProperty().addListener(new ChangeListener<Number>() {
            @Override
            public void changed(ObservableValue<? extends Number> ov, Number t, Number t1) {
            	prefs.putDouble("InfoTypeStyleSelect__treeTableColumn_id__PrefWidth", t1.doubleValue());
            }
        });
    	
    	treeTableColumn_name.setPrefWidth(prefs.getDouble("InfoTypeStyleSelect__treeTableColumn_name__PrefWidth", 300));

    	treeTableColumn_name.widthProperty().addListener(new ChangeListener<Number>() {
            @Override
            public void changed(ObservableValue<? extends Number> ov, Number t, Number t1) {
            	prefs.putDouble("InfoTypeStyleSelect__treeTableColumn_name__PrefWidth", t1.doubleValue());
            }
        });
    	
    	treeTableColumn_description.setPrefWidth(prefs.getDouble("InfoTypeStyleSelect__treeTableColumn_description__PrefWidth", 130));

    	treeTableColumn_description.widthProperty().addListener(new ChangeListener<Number>() {
            @Override
            public void changed(ObservableValue<? extends Number> ov, Number t, Number t1) {
            	prefs.putDouble("InfoTypeStyleSelect__treeTableColumn_description__PrefWidth", t1.doubleValue());
            }
        });
    	...
    	final TreeItem<TemplateSimpleItem> root;
   		root = new TreeItem<>(new TemplateSimpleItem(
   				0, themeId, "Темы", "это корень", TemplateSimpleItem.TYPE_STYLE, 0,0, new Date(), new Date(), "",""));
    ...
    private void addTreeItems(TreeItem<TemplateSimpleItem> ti) {
    	...
    	if (f != null) {
    		List<TemplateSimpleItem> styleList = conn.db.infoTypeStyleList (infoTypeId, f.getId());
	...
	private void saveState() {
		...
		//-------- size and position
		prefs.putDouble("stageInfoTypeStyleSelect_Width", dialogStage.getWidth());
		prefs.putDouble("stageInfoTypeStyleSelect_Height",dialogStage.getHeight());
		prefs.putDouble("stageInfoTypeStyleSelect_PosX",  dialogStage.getX());
		prefs.putDouble("stageInfoTypeStyleSelect_PosY",  dialogStage.getY());
		...
		if (treeTableView_styles.getSortOrder().size() > 0) {     // при сортировке по нескольким столбцам поменять if на for
			...
			prefs.put("stageInfoTypeStyleSelect_sortColumnId",currentSortColumn.getId());
			prefs.put("stageInfoTypeStyleSelect_sortType",currentSortColumn.getSortType().toString());
		} else {
			prefs.remove("stageInfoTypeStyleSelect_sortColumnId");
			prefs.remove("stageInfoTypeStyleSelect_sortType");
		}
	}
    ...
    private void handleButtonSelect() {
    	...
    	if (radioButton_inListStyle.isSelected()) {
    		...
    		styleSelected = conn.db.infoTypeStyleGet(si.getId());
